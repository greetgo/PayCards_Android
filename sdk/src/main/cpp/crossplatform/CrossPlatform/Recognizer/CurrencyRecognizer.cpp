...
CCurrencyRecognizer::CCurrencyRecognizer(const shared_ptr<IServiceContainer>& container) : _container(container)
{
...
CCurrencyRecognizer::~CCurrencyRecognizer()
{
}

void CCurrencyRecognizer::SetRecognitionMode(PayCardsRecognizerMode flag)
{
    _mode = flag;
}

void CCurrencyRecognizer::SetPathCurrencyLocalizationXModel(const string& path)
{
    _pathCurrencyLocalizationXModel = path;
}

void CCurrencyRecognizer::SetPathCurrencyLocalizationXStruct(const string& path)
{
    _pathCurrencyLocalizationXStruct = path;
}

void CCurrencyRecognizer::SetDelegate(const shared_ptr<IRecognitionCoreDelegate>& delegate)
{
    _delegate = delegate;
}

void CCurrencyRecognizer::SetPathCurrencyYLocalizationViola(const string& path)
{
    _pathCurrencyYLocalizationViola = path;
}

void CCurrencyRecognizer::SetPathCurrencySpaceCharModel(const string& path)
{
    _pathCurrencySpaceCharModel = path;
}

void CCurrencyRecognizer::SetPathCurrencySpaceCharStruct(const string& path)
{
    _pathCurrencySpaceCharStruct = path;
}

void CCurrencyRecognizer::SetPathCurrencyListTxt(const string& path)
{
    _pathCurrencyDictPath = path;
}

bool CCurrencyRecognizer::Deploy()
{
    if(auto factory = _factory.lock()) {

        bool cascadeFlag = false;

        if (_pathCurrencyYLocalizationViola.length() > 0) {
            cascadeFlag = _yCascade.load(_pathCurrencyYLocalizationViola);
        }

        _localizationXNeuralNetwork = factory->CreateNeuralNetwork("", _pathCurrencyLocalizationXStruct, _pathCurrencyLocalizationXModel);

        _spaceCharNeuralNetwork = factory->CreateNeuralNetwork("", _pathCurrencySpaceCharStruct, _pathCurrencySpaceCharModel);

        if (_pathCurrencyDictPath.length() > 0) {

            std::ifstream namesDict(_pathCurrencyDictPath);
            string fileContents { istreambuf_iterator<char>(namesDict), istreambuf_iterator<char>() };

            _currenciesDict.str(fileContents);
        }

        return _localizationXNeuralNetwork->IsDeployed() &&
                _spaceCharNeuralNetwork->IsDeployed() && cascadeFlag && _pathCurrencyDictPath.length() > 0;
    }
...
CCurrencyRecognizer::Process(cv::Mat &matrix, vector<cv::Mat> &charSamples,
                         std::string &postprocessedCurrency) {
    vector<Rect> charRects;

    Mat nameMat = matrix(nameWindowRect);

    _yCascade.detectMultiScale(nameMat, charRects, 1.05, 3, 0 | CV_HAAR_SCALE_IMAGE,
                               cv::Size(charSize.width - 1, charSize.height - 1),
                               cv::Size(charSize.width + 1, charSize.height + 1));
    ...
    Rect roi = Rect(0, cvRound(minY), nameMat.cols, charSize.height * 2 + 4);
    ...
    if (CUtils::ValidateROI(nameMat, roi)) {
        if (auto factory = _factory.lock()) {
            ...
            _yCascade.detectMultiScale(xLoc, charRects, 1.05, 3, 0 | CV_HAAR_SCALE_IMAGE,
                                       cv::Size(charSize.width - 2, charSize.height - 2),
                                       cv::Size(charSize.width + 2, charSize.height + 2));
            ...
            if (xCoord.size() > 0) {
                ...
                cv::Rect doubleRect = cv::Rect(cvRound(xCoord[bestIndex]), 27, 44, 28);
                ...
                if (CUtils::ValidateROI(nameMat, doubleRect)) {
                    ...
                    float newBest = xCoord[bestIndex] +
                                    (data.at(0).second + data.at(1).second) * 22.0; //(x0+x1)/2 * 44
                    ...
                    for (float x : charRects) {
                        ...
                        chars.push_back(xLoc(cv::Rect(roundedX, 26, 20, 29)));
                        count++;
                    ...
                    for (INeuralNetworkResultList::ResultIterator it = charResult->Begin();
                         it != charResult->End(); ++it) {
                        ...
                        resultStrProb0 << fixed << setprecision(2)
                                       << resultItem->GetMaxProbability();
                        resultStrProb0 << ";";
                    ...
                    if (charsCount > 5 && non < maxDoubtfulCount) {
                        ...
                        resultStr.erase(resultStr.begin(),
                                        std::find_if(resultStr.begin(), resultStr.end(), std::not1(
                                                std::ptr_fun<int, int>(std::isspace))));
                        resultStr.erase(std::find_if(resultStr.rbegin(), resultStr.rend(),
                        ...
                        for (string &chunk : chunks) {
                            ...
                            _currenciesDict.seekg(0, ios::beg);

                            while (_currenciesDict >> dictName) {
                            ...
                            if (minSimilarity != 0 && minSimilarity < 2 &&
                                minChunk.size() == chunk.size()) {
                                chunk = minChunk;
                            }
                        }
...
int CCurrencyRecognizer::FilterCoords(const std::vector<float>& xCoords,
...
void CCurrencyRecognizer::Predict(const vector<Mat>& matrixes, shared_ptr<INeuralNetworkResultList>& neuralNetworkResultList, const shared_ptr<INeuralNetwork>& neuralNetwork)
{
    if(auto factory = _factory.lock()) {
...
int CCurrencyRecognizer::LevenshteinDistance(const string& src, const string& dst)
{
