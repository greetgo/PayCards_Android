...
#define kCurrencyRecognitionAttempts  3
...
static int currencyRecognitionAttemptsCount = 0;
...
CRecognitionCore::CRecognitionCore(const shared_ptr<IRecognitionCoreDelegate> &delegate,
                                   const shared_ptr<ITorchDelegate> &torchDelegate) : _delegate(
        delegate), _orientation(PayCardsRecognizerOrientationPortrait),
                                                                                      _mode(PayCardsRecognizerModeNone),
                                                                                      _deployed(
                                                                                              false) {
    _isIdle.store(false);
    _isBusy = false;
...
void CRecognitionCore::SetRecognitionMode(PayCardsRecognizerMode flag) {
    ...
    if (_mode & PayCardsRecognizerModeCurrency) {
        _currencyRecognizer = _serviceContainerPtr->resolve<ICurrencyRecognizer>();
        if (auto currencyRecognizer = _currencyRecognizer.lock()) {
            currencyRecognizer->SetRecognitionMode(flag);
        }
    }
}
...
void CRecognitionCore::Deploy() {
    ...
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetDelegate(_delegate);
        if (!currencyRecognizer->Deploy()) return;
    }
    _deployed = true;
}
...
void CRecognitionCore::SetPathCurrencyYLocalizationViola(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencyYLocalizationViola(path);
    }
}

void CRecognitionCore::SetPathCurrencyLocalizationXModel(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencyLocalizationXModel(path);
    }
}

void CRecognitionCore::SetPathCurrencyLocalizationXStruct(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencyLocalizationXStruct(path);
    }
}

void CRecognitionCore::SetPathCurrencySpaceCharModel(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencySpaceCharModel(path);
    }
}

void CRecognitionCore::SetPathCurrencySpaceCharStruct(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencySpaceCharStruct(path);
    }
}

void CRecognitionCore::SetPathCurrencyListTxt(const string &path) {
    if (auto currencyRecognizer = _currencyRecognizer.lock()) {
        currencyRecognizer->SetPathCurrencyListTxt(path);
    }
}
...
void CRecognitionCore::ProcessFrame(DetectedLineFlags &edgeFlags, void *bufferY,
    ...
    if (!IsIdle() && _deployed) {
        ...
        Mat frameMatY = cv::Mat(1280, 720, CV_8UC1,
                                bufferY); //put buffer in open cv, no memory copied
                ...
                if (auto frameStorage = _frameStorage.lock()) {
                    ...
                    if (edgeFlags & DetectedLineTopFlag && edgeFlags & DetectedLineBottomFlag
                        ...
                        if (!_isBusy) {
                            ...
                            if (!(recognitionResult->GetRecognitionStatus() &
                                  RecognitionStatusNumber)) {
                                frameStorage->SetRawY(frameMatY.data, bufferUV, _edges,
                                                      _orientation);
...
//        // currency
//        if ((_mode & PayCardsRecognizerModeCurrency) &&
//            !(recognitionResult->GetRecognitionStatus() & RecognitionStatusCurrency) &&
//            (currencyRecognitionAttemptsCount < kCurrencyRecognitionAttempts)) {
//            LOGI("Recognize(): RecognizeCurrency()");
//            if (!RecognizeCurrency()) {
//                FinishRecognition();
//                return;
//            }
//        }
        ...
        if (_mode & PayCardsRecognizerModeDate || _mode & PayCardsRecognizerModeNumber ||
            _mode & PayCardsRecognizerModeCurrency) {
            _delegate->RecognitionDidFinish(recognitionResult,
                                            (PayCardsRecognizerMode) (
                                                    PayCardsRecognizerModeNumber |
                                                    PayCardsRecognizerModeDate |
                                                    PayCardsRecognizerModeCurrency));
        ...
        currencyRecognitionAttemptsCount = 0;
...
bool CRecognitionCore::RecognizeCurrency() {
    shared_ptr<INeuralNetworkResultList> result;
    ...
    if (auto frameStorage = _frameStorage.lock()) {
        if (auto currencyRecognizer = _currencyRecognizer.lock()) {
            if (auto recognitionResult = _recognitionResult.lock()) {
                ...
                result = currencyRecognizer->Process(frame, samples, postprocessedCurrency);
                LOGI("Recognize(): RecognizeCurrency(): %s", postprocessedCurrency.c_str());

                currencyRecognitionAttemptsCount++;
                if (result) {
                    recognitionResult->SetCurrencyResult(result);
                    recognitionResult->SetRecognitionStatus(RecognitionStatusCurrency);
                }
                recognitionResult->SetPostprocessedCurrency(postprocessedCurrency);
        ...
        if (auto numberRecognizer = _numberRecognizer.lock()) {
            if (auto recognitionResult = _recognitionResult.lock()) {
                ...
                if (result) {
                    ...
                    recognitionResult->SetRecognitionStatus(RecognitionStatusNumber);
                }
                if (auto torchManager = _torchManager.lock()) {
        ...
        if (auto dateRecognizer = _dateRecognizer.lock()) {
            if (auto recognitionResult = _recognitionResult.lock()) {
                ...
                if (result) {
                    ...
                    recognitionResult->SetRecognitionStatus(RecognitionStatusDate);
                }
            }
        }
    }
