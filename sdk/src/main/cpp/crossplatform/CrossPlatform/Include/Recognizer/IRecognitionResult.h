...
enum
{
    ...
    RecognitionStatusCurrency      = (1 << 4),
    RecognitionStatusCompleted      = (1 << 5),
};
...
public:
    ...
    virtual shared_ptr<INeuralNetworkResultList> GetCurrencyResult() const = 0;
...
//    virtual vector<cv::Mat> GetNameSamples() const = 0;
    ...
    virtual std::string GetPostprocessedCurrency() const = 0;
    ...
    virtual void SetCurrencyResult(shared_ptr<INeuralNetworkResultList> result) = 0;
...
//    virtual void SetNameSamples(vector<cv::Mat> samples) = 0;
    ...
    virtual void SetPostprocessedCurrency(std::string name) = 0;
};
